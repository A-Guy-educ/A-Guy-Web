...
4. For **every** candidate, compute commits-behind. Issue the compare calls **sequentially, one PR at a time**, and after each call **immediately log the pair** so the association is unambiguous. Do not batch these in parallel — small numeric responses are easily mis-paired across parallel bash output. Use this exact pattern for each candidate `n` with base `B` and head `H`:

   ```
   behind=$(gh api repos/{owner}/{repo}/compare/B...H --jq '.behind_by')
   echo "PR #n base=B head=H behind=$behind"
   ```

   Do not skip any candidate from this enumeration; the behind-count check decides per PR, not the prompt. Before deciding actions, write a clearly labeled table mapping every candidate PR to (head SHA, behind, statusCheckRollup state, prior perPr entry).
...
- `lastActionAt` must be the **literal current UTC time** at the moment the comment was posted, in ISO-8601 with seconds precision (e.g. `2026-05-04T14:57:53Z`). Do not round, snap to the next half-hour, or otherwise transform it — the 6-hour cooldown is computed from this value verbatim.
