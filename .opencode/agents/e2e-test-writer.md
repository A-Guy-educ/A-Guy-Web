...
**CRITICAL**: These tests are committed to `tests/e2e/` but are **NOT run in the Cody pipeline**. They run in the separate CI `e2e-system-tests` job when the PR is opened. This means you do NOT need to worry about them passing right now — write thorough tests that cover the user-facing behavior.
...
/**
 * @cody-e2e Auto-generated by Cody pipeline
 * @task <task-id>
 * @date <ISO date>
 */
