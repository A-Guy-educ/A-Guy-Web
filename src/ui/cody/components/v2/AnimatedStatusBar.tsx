...
const statusStyles: Record<ColumnId, StatusBarStyle> = {
  ...
  'gate-waiting': {
    ...
    animation: 'animate-cody-breathe',
  },
  review: {
      ...
      <div className={cn('h-1.5 rounded-full overflow-hidden relative', style.barTrack)}>
        ...
        >
          ...
          {(task.column === 'building' || task.column === 'retrying') && (
            <div className="absolute right-0 top-0 h-full w-3 bg-gradient-to-l from-white/40 to-transparent rounded-r-full animate-cody-leading-edge" />
          )}
        </div>
        ...
        {task.column === 'building' && (
          <div className="absolute inset-0 overflow-hidden rounded-full">
            <div className="h-full w-1/4 bg-gradient-to-r from-transparent via-white/15 to-transparent animate-cody-shimmer" />
          </div>
        )}
        ...
        {task.column === 'gate-waiting' && (
          <div className="absolute inset-0 overflow-hidden rounded-full">
            <div
              className="h-full bg-gradient-to-r from-transparent via-amber-300/20 to-transparent animate-cody-breathe-overlay"
              style={{ width: `${percent}%` }}
            />
          </div>
        )}
...
function BarLabel({ task, style: _style }: { task: CodyTask; style: StatusBarStyle }) {
  const displayState = derivePipelineDisplayState(task)
  const totalStages = ALL_STAGES.length
...
function getFailedPercent(task: CodyTask): number {
