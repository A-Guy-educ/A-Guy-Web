...
|------|-------|---------|---------|------|
| approval-gate | cto | 15m | broken | state file never created (0 commits); kody-job-next-state block well-structured but procedure never writes it |
...
| dead-code-sweep | ceo | 7d | broken | script never implemented; state at legacy .kody/jobs/ path |
...
| dev-ci-health | ceo | 1h | broken | kody-job-next-state present but missing lastRunISO/nextEligibleISO fields |
...
| flaky-test-quarantine | ceo | 1h (disabled) | warn | no kody-job-next-state block; disabled=true so idle by design |
...
| security-audit | ceo | 1d (disabled) | warn | no kody-job-next-state block; disabled=true so idle by design |
...
| type-debt | ceo | 1d (disabled) | warn | no kody-job-next-state block; state never created; disabled=true so idle by design |