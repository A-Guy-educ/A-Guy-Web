...
Cycle 5 — 0 healthy, 11 warn, 14 broken of 26 duties.
...
| cleanup-branches | coo | manual | broken | no per-tick procedure; describes policy only with no executable steps; allowed-commands includes git push (non-gh) but no gh write method for branch deletion |
...
| job-gap-scan | ceo | none (no every: field) | broken | state.json persisted to .kody/jobs/ (legacy path) while body says .kody/duties/; migration moved file but script still writes to old location; two non-identical state files exist |
