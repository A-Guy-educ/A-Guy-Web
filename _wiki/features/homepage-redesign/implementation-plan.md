...
export const UserProgress: CollectionConfig = {
  ...
  indexes: [{ fields: { user: 1, gradeLevel: 1 } }],
  timestamps: true,
}
```
...
export async function syncLocalProgressToDatabase(userId: string, gradeLevel: string) {
  const localProgress = getLocalProgress()
  ...
  if (existing) {
    const mergedRecords = mergeProgressRecords(existing.progressRecords || [], localProgress)
...
function mergeProgressRecords(dbRecords: any[], localRecords: LocalProgressRecord[]): any[] {
...
- **Tailwind CSS classes** - All utility classes (flex, grid, text-_, bg-_, etc.)
