...
export function useExercisesPager({
  ...
  // Reorder blocks: content pages first, then exercises (relative order preserved within groups)
  const contentPages = rawBlocks.filter((b) => b.type === 'contentPage')
  const exerciseBlocks = rawBlocks.filter((b) => b.type === 'exercise')
  const resolvedBlocks: ResolvedLessonBlock[] = [...contentPages, ...exerciseBlocks]
  ...
  const [pageState, setPageState] = useState<PageState>(() => {
    if (resolvedBlocks.length === 0) return { type: 'outro', pageNumber: 0 }
  ...
  const handleStart = useCallback(() => {
    if (resolvedBlocks.length === 0) {
      setPageState({ type: 'outro', pageNumber: 0 })
      return
    }
