...
import type { PrototypeElement } from '@/infra/qa/schema'
...
export function PrototypePanel({
  ...
  useEffect(() => {
    async function loadPrototypes() {
      try {
        const response = await fetch('/api/cody/scenario/prototypes')
        if (response.ok) {
  ...
  useEffect(() => {
    async function loadElements() {
      ...
      setLoading(true)
      try {
        const response = await fetch(`/api/cody/scenario/prototypes/${selectedPrototype}`)
        if (response.ok) {
          const data = await response.json()
  ...
  const handleUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const listResponse = await fetch('/api/cody/scenario/prototypes')
        if (listResponse.ok) {
