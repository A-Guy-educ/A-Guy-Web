...
export async function generateAssistantReply(args: {
  ...
  const localeInstruction = args.locale === 'he' ? 'IMPORTANT: Respond in Hebrew. ' : ''
  const system = `${localeInstruction}You are A-Guy, a concise math tutor. Help the student with clear steps, in the same language they use when possible.`
