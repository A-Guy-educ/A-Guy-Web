...
import type { CodyNotification, NotificationPrefs, NotificationType } from './types'
import { DEFAULT_PREFS, NOTIFICATION_META } from './types'

const STORAGE_KEY = 'cody-notifications'
const PREFS_KEY = 'cody-notification-prefs'
const MAX_NOTIFICATIONS = 50

function loadNotifications(): CodyNotification[] {
  if (typeof window === 'undefined') return []
  try {
    const raw = localStorage.getItem(STORAGE_KEY)
    return raw ? (JSON.parse(raw) as CodyNotification[]) : []
  } catch {
    return []
  }
}

function saveNotifications(items: CodyNotification[]): void {
  if (typeof window === 'undefined') return
  try {
...
export interface UseNotificationStoreReturn {
  notifications: CodyNotification[]
  unreadCount: number
...
export function useNotificationStore(): UseNotificationStoreReturn {
  const [notifications, setNotifications] = useState<CodyNotification[]>([])
  ...
  const addNotification = useCallback(
    (
      ...
      const notification: CodyNotification = {
