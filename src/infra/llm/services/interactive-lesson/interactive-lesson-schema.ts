 ...
 *
 * @ai-summary Passed directly to Gemini's responseSchema API (not Genkit); Gemini's responseSchema implementation silently ignores `$ref`, `$defs`, and `oneOf` with discriminator — these constructs in the schema will be dropped and cause the call to fail with a schema validation error. Passed directly to Gemini's responseSchema API — Gemini must be able to express this schema in OpenAPI 3.0 subset. Keep it FLAT: no $ref, $defs, or oneOf with discriminator. The `stripUnsupportedKeys` helper only removes $schema and additionalProperties; other JSON-Schema-only keywords will silently pass through and cause Gemini API errors. Service-side Zod validators run AFTER as a safety net for rare schema-ignore cases. Passed directly to Gemini's responseSchema API — must be kept FLAT (no $ref, $defs, oneOf) since Gemini's responseSchema accepts only a strict OpenAPI 3.0 subset. The `stripUnsupportedKeys` helper only removes `$schema` and `additionalProperties`; other JSON-Schema-only keywords pass through and cause Gemini API errors.
 *
