...
|------|-------|---------|---------|------|
| approval-gate | cto | 15m | broken | state file never created; procedure has no write path for state; allowed commands are read-only |
...
| clear-empty-goals | cto | 7d | broken | 0-step body; no kody-job-next-state block; state file never created |
...
| dead-code-sweep | cto | 1d | broken | script never implemented; state at legacy .kody/jobs/ path |
...
Cycle 16 completed full sweep. Cycle 17 started.