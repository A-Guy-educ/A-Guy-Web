 ...
 *
 * @ai-summary The prompt instructs raw JSON output only; any wrapping from the model causes parse failure — callers must strip markdown fences before calling JSON.parse. The prompt instructs the LLM to accept equivalent forms (3.14 ≈ π, x+x = 2*x). Return format must be EXACTLY JSON — no markdown fences. If the response isn't valid JSON, the service returns an error (no retry). Language-agnostic: accepts Hebrew or English as long as meaning matches.
 */
