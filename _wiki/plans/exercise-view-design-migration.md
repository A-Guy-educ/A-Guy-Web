...
export function ResizablePane({
  ...
  return (
    ...
    >
      ...
      {/* Resizer Handle */}
      <div className={resizerClasses} onMouseDown={handleMouseDown} onTouchStart={handleMouseDown}>
        <div className={cn('bg-[#cbd5e0] rounded-full', isVertical ? 'w-1 h-10' : 'w-10 h-1')} />
      </div>
...
export function ExerciseWorkspace({
  ...
  return (
    <div className="fixed inset-0 bg-background z-50 flex flex-col overflow-hidden">
      ...
      >
        {/* PDF Viewer Section */}
        <div className="bg-[#525659] overflow-hidden">{pdfContent}</div>
...
export function ChatInterface() {
  const t = useTranslations('courses')
  const [messages, setMessages] = useState<Message[]>([{ role: 'ai', content: t('chatWelcome') }])
  ...
  const handleSend = () => {
    ...
    setTimeout(() => {
      setMessages((prev) => [...prev, { role: 'ai', content: t('chatAIResponse') }])
    }, 1000)
  }
...
const formulas = {
  algebra: [
    ...
    {
      label: 'נוסחת השורשים',
      template: 'x_{1,2} = \\frac{-b \\pm \\sqrt{b^2-4ac}}{2a}',
      offset: 0,
    },
    { label: '(a-b)²', template: '(a-b)^2 = a^2-2ab+b^2', offset: 0 },
