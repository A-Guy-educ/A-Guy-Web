...
async function extractFromImage(input: ImageToExerciseInput): Promise<ImageToExerciseResponse>
```
...
async function chatWithExerciseHelper(input: ExerciseChatInput): Promise<ExerciseChatResult>
```
...
export async function myNewService(input: MyServiceInput): Promise<MyServiceResult> {
  try {
    const client = getGeminiClient()
