 ...
 * @ai-summary Session management hook for Cody global chat - CRUD operations with localStorage persistence
 */
import { useState, useEffect, useCallback, useMemo } from 'react'
import type { AgentId } from '../agents'
import type { ChatMessage, GlobalChatStore, SessionMeta } from '../chat-types'

const STORAGE_KEY = 'cody-sessions-v2'
...
function loadStore(): GlobalChatStore {
  ...
  try {
    ...
    const v1Data = localStorage.getItem('cody-global-chat')
    if (v1Data) {
      ...
      // Remove legacy key
      localStorage.removeItem('cody-global-chat')
      return migrated
    }
  } catch (error) {
