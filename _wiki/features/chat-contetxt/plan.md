...
async function provisionVectorIndex() {
  ...
  const requiredVars = ['MONGODB_URI', 'ATLAS_PROJECT_ID', 'ATLAS_PUBLIC_KEY', 'ATLAS_PRIVATE_KEY']
...
async function extractAndPersist(payload, userId, conversationId, conversation) {
  const candidates = await extractMemoryCandidates(conversation.messages, conversation.summary)

  if (candidates.length > 0) {
    await persistMemoryItems(payload, userId, conversationId, candidates, new Date(), 'model')
  }
}
```
