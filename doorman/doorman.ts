...
function decodeTicket(ticket: string): TicketPayload | null {
  try {
    const payload = JSON.parse(Buffer.from(ticket, 'base64url').toString('utf8')) as unknown
...
function verifyAndGetSession(ticket: string, key: Buffer): boolean {
  ...
  const expectedSig = crypto.createHmac('sha256', key).update(subject).digest('hex').slice(0, 32)
...
function setSessionCookie(res: http.ServerResponse): void {
  res.setHeader(
    'Set-Cookie',
    `${COOKIE_NAME}=1; Path=/; HttpOnly; SameSite=None; Secure; Max-Age=${COOKIE_MAX_AGE}`,
  )
}
...
function proxyRequest(
  ...
  for (const h of hopByHop) {
    delete (options.headers as Record<string, string | string[] | undefined>)[h]
  }
...
const server = http.createServer((req, res) => {
  ...
  if (!key) {
    console.error('[doorman] KODY_PREVIEW_VERIFY_KEY not set — rejecting all requests')
